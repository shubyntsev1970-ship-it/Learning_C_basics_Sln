...
namespace Learning_C_basics_App
{
    public static partial class Program
    {
        ...
        // Статические поля класса. Как работает ключевое слово static
        private static void Lesson_059()
        {
            Console.WriteLine("Hello from Lesson_059  Статические поля класса. Как работает ключевое слово static");
            Console.WriteLine();

            // Статические поля класса - это поля, которые принадлежат самому классу, а не его экземплярам.
            // Они общие для всех объектов класса и могут быть доступны без создания экземпляра класса.
            // Ключевое слово static используется для объявления статических членов класса.

            Console.WriteLine($"Количество созданных объектов класса Point6: {Point6.ObjectCount}");
            Point6 point1 = new Point6();
            Console.WriteLine($"Количество созданных объектов класса Point6: {Point6.ObjectCount}");
            Point6 point2 = new Point6();
            Console.WriteLine($"Количество созданных объектов класса Point6: {Point6.ObjectCount}");
            Console.WriteLine();

            MyClass1 myClass1 = new MyClass1();
            myClass1.SetB(9); // Установка значения статического поля b через экземпляр класса myClass1.
                              // Это не рекомендуется, но допустимо в C#

            MyClass1 myClass2 = new MyClass1();
            myClass2.PrintB(); // Вывод значения статического поля b через другой экземпляр класса myClass2.
                               // Это покажет значение 9, потому что b - это статическое поле,
                               // общее для всех экземпляров класса MyClass1.

            Console.WriteLine(new string('-', 120));
        }
        internal class Point6
        {
            public int x;
            public int y;

            // Статическое поле для подсчета количества созданных объектов класса Point6
            public static int ObjectCount = 0;

            // Конструктор класса Point6, который увеличивает счетчик объектов при каждом создании нового объекта
            public Point6()
            {
                ObjectCount++;
            }
        ...
        // Статические методы. Статические свойства. Как работает ключевое слово static
        /// <summary>
        /// 
        /// </summary>
        private static void Lesson_060()
        {
            Console.WriteLine("Hello from Lesson_060  Статические методы. Статические свойства. Как работает ключевое слово static");
            Console.WriteLine();

            // Статические методы - это методы, которые принадлежат самому классу, а не его экземплярам.
            // Они могут быть вызваны без создания экземпляра класса и обычно используются для выполнения операций,
            // которые не зависят от конкретного объекта класса.
            // Статические свойства - это свойства, которые принадлежат самому классу, а не его экземплярам.

            MyClass2 myClass = new MyClass2();
            // myClass.Foo();   // Нельзя вызвать статический метод Foo() через экземпляр класса myClass,
            // так как он объявлен как static.
            // Ошибка! Невозможно обратиться к методу 'MyClass2.Foo()', так как он объявлен как static
            MyClass2.Foo();     // Правильный способ вызвать статический метод Foo() - через имя класса MyClass2.

            //MyClass2.Bar();   // Нельзя вызвать нестатический метод Bar() через имя класса MyClass2,
            // так как он не объявлен как static.
            // Ошибка! Невозможно обратиться к методу 'MyClass2.Bar()', так как он не является статическим

            myClass.Bar(); // Правильный способ вызвать нестатический метод Bar() - через экземпляр класса myClass,
                           // который был создан ранее.

            //myClass.a = 1; // Нельзя установить значение статического поля a через экземпляр класса myClass,
            // так как оно объявлено как static.
            // Ошибка! Невозможно обратиться к полю 'MyClass2.a', так как оно объявлено как static

            //myClass.c = 1; // Нельзя установить значение статического свойства c через экземпляр класса myClass,
            // так как оно объявлено как static.
            // Ошибка! Невозможно обратиться к члену 'MyClass2.c' через экземпляр класса,
            // так как оно объявлено как static

            MyClass2.C = 11; // Правильный способ установить значение статического свойства C - через имя класса MyClass2,
                             // так как оно объявлено как static.

            Console.WriteLine(MyClass2.C); // Правильный способ получить значение статического свойства C -
                                           // через имя класса MyClass2, так как оно объявлено как static.
        ...
        class MyClass2
        {
            ...
            // Статическое свойство D с явной реализацией
            ...
            // Статическое свойство c с автоматической реализацией
            // Автоматическое свойство c, которое автоматически создает скрытое статическое поле для хранения значения c
            public static int C { get; set; }

            public static void Foo()
            {
                Console.WriteLine("Вызван статический метод Foo()");

                a++; // Статическое поле a может быть доступно внутри статического метода Foo(),
                     // так как оно принадлежит классу MyClass2
                     
                //b++; // Невозможно обратиться к нестатическому полю b внутри статического метода Foo(),
                     // так как нестатические поля принадлежат экземплярам класса, а не самому классу.
                     // Ошибка! Невозможно обратиться к полю 'MyClass2.b', так как оно не является статическим
                
                //Bar(); // Невозможно вызвать нестатический метод Bar() внутри статического метода Foo(),
                     // так как нестатические методы принадлежат экземплярам класса, а не самому классу.
                     // Ошибка! Невозможно обратиться к методу 'MyClass2.Bar()', так как он не является статическим
            }

            public void Bar()
            {
                Console.WriteLine("Вызван нестатический метод Bar()");

                a = 5; // Статическое поле a может быть доступно внутри нестатического метода Bar()
                b = 10; // Нестатическое поле b может быть доступно внутри нестатического метода Bar()
                Console.WriteLine("Вызываю статический метод Foo() из нестатического метода Bar()");
                Foo(); // Статический метод Foo() может быть вызван внутри нестатического метода Bar(),
                       // так как статические методы принадлежат классу MyClass2 и доступны для всех его методов,
                       // включая нестатические.
            }
        }
    }
}
